...
- 工单自动创建：`ticket`
- 工单最小状态机：支持 `NEW / ACKED / IN_PROGRESS / RESOLVED / CLOSED` 状态流转，并记录 `ack_at / resolved_at / closed_at`
- 通知结果留痕：`notify_record`
- 审计流水留痕：`operation_log`
- 最小反馈闭环：人工反馈落库 `feedback_sample`，支持 `valid / false_positive / duplicate` 等最终标签，并与 `group / ticket` 关联
- SQLite 本地持久化
- In-Memory 模式本地演示
- Redis Stream 模式可选启用
- Swagger 分组与联调示例优化
...
6. **状态层**通过最小状态机描述工单从创建到关闭的处理过程
7. **反馈层**允许人工补充最终结论，形成“自动判断 + 人工校正”的最小闭环
8. **审计层**对关键动作全程留痕，便于查询、复盘与演示  
...
状态流转通过接口更新，并记录关键时间字段，例如：

- `ack_at`
- `resolved_at`
- `closed_at`

这样做的价值在于：即使当前版本仍是 MVP，也不只是“生成一张工单”，而是能表达最基本的处置过程，并配合审计日志实现可查询、可验证、可复盘。

### 7. 工单最小状态机

AIAlert 当前没有直接追求完整 ITSM 能力，而是优先实现了一个最小可演示、可审计的工单状态机。其核心目标是回答两个问题：

- 告警被开单之后，是否有人接手处理
- 一个事件从出现到恢复，系统能否保留最小处置轨迹

当前状态流转为：

`NEW -> ACKED -> IN_PROGRESS -> RESOLVED -> CLOSED`

这一设计虽然简单，但已经能支持以下场景：

- 演示告警事件的基础处置流程
- 验证关键时间节点是否被正确记录
- 在 `operation_log` 中审计每次状态变更
- 为后续扩展 SLA、升级策略、多人协作预留结构基础

### 8. 最小反馈闭环

在自动接入、聚合、AI 分析、工单和通知链路之外，项目补充了最小反馈闭环，用于接收人工最终结论。

当前通过 `feedback_sample` 保存人工反馈，核心字段包括：
...
这一能力的意义不在于“多了一张表”，而在于系统具备了从自动处理走向人工校正的最小闭环能力：

- 前端链路完成自动接入、聚合、AI 判定、工单和通知
- 后端允许人工补充最终结论
- 反馈既可用于误报复盘，也可为后续规则优化和 AI 样本沉淀提供基础
- 每次反馈提交都会写入 `operation_log`，保证人工干预过程可审计

因此，当前系统已经不是单纯“自动判断”的链路，而是具备了“自动决策 + 人工反馈 + 审计留痕”的最小闭环能力。
...
- 支持通过接口更新工单状态
- 支持 `NEW / ACKED / IN_PROGRESS / RESOLVED / CLOSED` 顺序流转
- `ack_at / resolved_at / closed_at` 等关键时间字段会随状态变化正确填充
- `operation_log` 中可查询对应的状态流转审计记录

这部分验证说明系统已经具备最小处置流程表达能力，而不只是“创建工单后停留在静态记录”。

### 最小反馈闭环验证

已验证人工反馈相关能力：

- 支持通过 `POST /feedback` 提交人工最终结论
- 支持通过 `GET /feedback` 按 `group_id / ticket_id / final_label` 查询反馈记录
- 反馈数据可正确写入 `feedback_sample`
- 每次提交反馈都会写入 `operation_log`
- 可完成“自动处理结果 + 人工复核结论 + 审计留痕”的最小闭环

这部分验证说明项目已经具备从自动判断走向人工校正的基础能力，可用于后续误报分析、规则优化与 AI 样本沉淀。
...
- `ai_analysis_result` 记录 AI 初步判断
- `ticket` 记录处置执行与状态变化
- `notify_record` 记录通知结果
- `feedback_sample` 记录人工最终结论
- `operation_log` 记录关键动作与状态流转
...
5. `GET /ai_results`  
   查看 AI 输出是否已结构化落库，区分 `success / fallback / suppress`

6. `GET /tickets`  
   查看是否生成工单

7. `PATCH /tickets/{ticket_id}/status`  
   演示工单从 `NEW` 到 `ACKED / IN_PROGRESS / RESOLVED / CLOSED` 的最小状态流转

8. `GET /notify_records`  
   查看通知是否发送或被抑制
...
10. `GET /feedback`  
    查询反馈是否成功落库，并验证 `group_id / ticket_id / final_label` 过滤能力

11. `GET /operation_logs`  
    查看关键动作是否有完整审计留痕，包括 AI 抑制、工单状态流转和反馈提交

这样一套演示下来，项目的“从接入到处置，再到人工反馈与审计”的闭环会比较清晰。
...
对于后续继续扩展规则、消息解耦、通知集成、工单协同、反馈学习与 AI 增强，这个版本已经具备较好的基础。
